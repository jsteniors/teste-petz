...
public class ApiControllerAdivice {
	...
	public ApiErrorResponse handle(HttpMessageNotReadableException exception) throws IOException {
    	logger.error("Invalid Format Errors: {}", exception.getMostSpecificCause().getLocalizedMessage());
	...
	public ApiErrorResponse handle(MethodArgumentNotValidException exception) {
    	logger.error("Validation Errors: {}", exception.getMessage());
    ...
    @ExceptionHandler({BusinessException.class, ExistingEntityException.class})
